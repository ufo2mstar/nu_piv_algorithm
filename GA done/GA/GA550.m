function [xopt,fopt,stats,nfit,fgen,lgen,lfit,generation,xval] = GA550(fun, ...
...
for generation = 1:max_gen+1,
    ...
    for i = 1:pop_size,
        x = x_pop(i,:);
        fitness(i) = eval([evalstr,')']);  %%naren second 10/29
% 			ii=ii+1; %% naren third
			xval(nfit+1,:)=[generation,x,fitness(i)]; %% naren third
